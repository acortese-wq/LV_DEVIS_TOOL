...
(function () {
  ...
  var PHDEF = [["31", ["Vorprojekt", "Avant-projet", "Progetto di massima", "Preliminary design"], 12], ["32", ["Bauprojekt", "Projet de l'ouvrage", "Progetto definitivo", "Construction design"], 22],
    ["33", ["Bewilligungsverfahren", "Procédure d'autorisation", "Procedura d'autorizzazione", "Permit procedure"], 2], ["41", ["Ausschreibung, Vergabe", "Appel d'offres, adjudication", "Appalto, aggiudicazione", "Tender, award"], 8],
    ["51", ["Ausführungsprojekt", "Projet d'exécution", "Progetto esecutivo", "Detailed design"], 18], ["52", ["Ausführung (Bauleitung)", "Exécution (direction des travaux)", "Esecuzione (direzione lavori)", "Execution (site supervision)"], 33],
    ["53", ["Inbetriebnahme, Abschluss", "Mise en service, achèvement", "Messa in servizio, chiusura", "Commissioning, close-out"], 5]];
  function newRec(tab, n) {
    ...
    switch (tab) {
      ...
      case "hon": r = { name: nm("Ingenieurhonorar"), src: "lv", B: "", n: "1", r: "1", u: "1", h: "140", nk: "0", ph: PHDEF.map(function (p) { return { k: p[0], on: true, q: String(p[2]) }; }) }; break;
    }
    r.x = {}; r.free = [];
    return r;
  }
  function defaults() {
    ...
    return { v: 2, lang: "de", tab: "graben", kopf: { projekt: "", ort: "", bearb: "", datum: today(), rolle: "anv", devisNr: "", lieferant: "", vertrag: "", termin: "", belagLief: false, sap: "", bem: "" },
      ...
      par: { deckG: "0.03", deckS: "0.035", deckK: "0.04", gehMin: "0.06", gehMax: "0.12", strMin: "0.09", strMax: "0.20", kanMin: "0.12", kanMax: "0.30", z1: "0.075", z2: "7.23" },
  ...
  var F = {
    ...
    hon: [{ sec: "hB" }, { id: "name", t: "text" }, { id: "src", t: "sel", opts: ["lv", "man"], struct: 1 }, { t: "honB" },
      { sec: "s_hfak" }, { id: "n", t: "num", half: 1, labKey: "f_hn" }, { id: "r", t: "num", half: 1, labKey: "f_hr" }, { id: "u", t: "num", half: 1, labKey: "f_hu" }, { id: "h", t: "num", half: 1, labKey: "f_hh" },
      { id: "nk", t: "num", labKey: "f_hnk" }, { sec: "s_hph" }, { t: "phases" }, { t: "honRes" }]
  };
  ...
  function phasesEl(cfg) {
    ...
    cfg.ph.forEach(function (p) {
      var d = PHDEF.filter(function (x) { return x[0] === p.k; })[0];
      var c = document.createElement("input"); c.type = "checkbox"; c.checked = !!p.on; c.setAttribute("aria-label", p.k);
      c.addEventListener("change", function () { p.on = c.checked; save(); refreshHon(); scheduleResult(); });
      var b = document.createElement("b"); b.textContent = p.k;
      var s = document.createElement("span"); s.textContent = d ? (d[1][L] || d[1][0]) : "";
      var q = document.createElement("input"); q.type = "text"; q.inputMode = "decimal"; q.value = p.q; q.setAttribute("aria-label", "q " + p.k);
      q.addEventListener("input", function () { p.q = q.value; save(); refreshHon(); scheduleResult(); });
  ...
  function honCalc(r, netto) {
    var B = r.src === "man" ? num(r.B, 0) : netto, q = r.ph.reduce(function (s, x) { return s + (x.on ? num(x.q, 0) : 0); }, 0);
    if (!(B > 0) || !(q > 0)) return { B: B, q: q, ok: false };
    var p = num(S.par.z1, 0.075) + num(S.par.z2, 7.23) / Math.cbrt(B), Tm = r2(B * p / 100 * num(r.n, 1) * q / 100 * num(r.r, 1) * num(r.u, 1)), H = r2(Tm * num(r.h, 0)), NK = r2(H * num(r.nk, 0) / 100);
    return { B: B, p: p, q: q, Tm: Tm, H: H, NK: NK, net: r2(H + NK), ok: true };
  }
  var lastOut = null;
  ...
  function honResHtml(r) {
    ...
    if (!h.ok) return '<p class="hint">⚠ ' + esc(tt("w_hon")) + "</p>";
    return '<table class="cost hres"><tbody><tr><td>B</td><td class="num">CHF ' + chf(h.B) + "</td></tr><tr><td>p = Z1 + Z2 / ∛B</td><td class=\"num\">" + h.p.toFixed(4) + " %</td></tr>" +
      ...
      "<tr><td>" + esc(tt("hHon")) + " (" + qf(h.Tm) + " h × " + chf(num(r.h)) + ')</td><td class="num">CHF ' + chf(h.H) + "</td></tr>" +
  ...
  function refreshHon() { var e = $("#honres"); if (e) e.innerHTML = honResHtml(rec()); var q = $("#qsum"); if (q) q.textContent = "q = " + qf(rec().ph.reduce(function (s, x) { return s + (x.on ? num(x.q, 0) : 0); }, 0)) + " %"; refreshFig(); }
  ...
  function honFig(r) {
    var h = honCalc(r, lvNetto()), tot = r.ph.reduce(function (s, x) { return s + Math.max(0, num(x.q, 0)); }, 0) || 1, x = 20, o = "";
    r.ph.forEach(function (p) { var w = Math.max(0, num(p.q, 0)) / tot * 720; if (w <= 0) return;
      o += '<rect x="' + x + '" y="40" width="' + w + '" height="40" fill="' + (p.on ? "var(--navy)" : "var(--rule-strong)") + '" stroke="#fff" stroke-width="2"/>' + (w > 26 ? '<text x="' + (x + w / 2) + '" y="65" class="sl sb" text-anchor="middle" style="fill:#fff">' + p.k + "</text>" : "") + (w > 40 ? FIG.tx(x + w / 2, 98, qf(num(p.q, 0)) + " %", "sm", "middle") : ""); x += w; });
    o += FIG.tx(20, 24, tt("s_hph"), "sl sb");
    o += h.ok ? FIG.tx(20, 134, "Tm = " + qf(Math.round(h.B)) + " × " + h.p.toFixed(4) + "/100 × " + qf(num(r.n, 1)) + " × " + qf(h.q) + "/100 × " + qf(num(r.r, 1)) + " × " + qf(num(r.u, 1)) + " = " + qf(h.Tm) + " h", "sl") +
      FIG.tx(20, 160, tt("hHon") + ": " + qf(h.Tm) + " h × CHF " + chf(num(r.h)) + " = CHF " + chf(h.H), "sl sb") : FIG.tx(20, 140, "⚠ " + tt("w_hon"), "sl");
  ...
  function renderForm() {
    ...
    F[S.tab].forEach(function (f) {
      ...
      if (f.t === "honRes") { var hr = document.createElement("div"); hr.id = "honres"; tgt.appendChild(hr); var hn = document.createElement("p"); hn.className = "hint"; hn.textContent = tt("hHint"); tgt.appendChild(hn); return; }
  ...
  function parEl() {
    ...
    var P = S.par, rows = [["dichte", S], ["mwst", S], ["z1", P], ["z2", P], ["deckG", P], ["deckS", P], ["deckK", P], ["gehMin", P], ["gehMax", P], ["strMin", P], ["strMax", P], ["kanMin", P], ["kanMax", P]];
  ...
  function renderResult() {
    ...
    if (hon.length) {
      ...
      html += "<h3>" + esc(tt("hSum")) + '</h3><table class="cost"><tbody>' + hon.map(function (x) { return "<tr><td>" + esc(x[0].name) + '<div class="rw">' + qf(x[1].Tm) + " h × CHF " + chf(num(x[0].h)) + (x[1].NK ? " + " + esc(tt("hNk")) + " " + chf(x[1].NK) : "") + " · B = " + chf(x[1].B) + '</div></td><td class="num">' + chf(x[1].net) + "</td></tr>"; }).join("") +
  ...
  function xlsxSheets(out) {
    ...
    var H = [hdr([tt("tab_hon"), "B", "q %", "Tm h", "h CHF", tt("hHon"), tt("hNk"), tt("hTot")])];
    S.recs.hon.forEach(function (r) { var h = honCalc(r, sm.netto); if (h.ok) H.push([{ v: r.name }, { v: h.B, s: XL.num }, { v: h.q, s: XL.num }, { v: h.Tm, s: XL.num }, { v: num(r.h), s: XL.num }, { v: h.H, s: XL.num }, { v: h.NK, s: XL.num }, { v: h.net, s: XL.sum }]); });
